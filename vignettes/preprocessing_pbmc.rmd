...
The input CITE-seq data can be downloaded from below.

```{sh, eval=FALSE}
wget https://atlas.fredhutch.org/data/nygc/multimodal/pbmc_multimodal.h5seurat
```

In this vignette, we start with a processed Seurat object (`pbmc.chomvar.annotated.rds`),
which can be downloaded
[here](https://www.dropbox.com/s/e4jbbmnhtq8kusq/pbmc.chromvar.annotated.rds?dl=0).
The script used to generate this file is available [here](https://github.com/yuchaojiang/TRIPOD/blob/main/vignettes/PBMC_multiomic_chromVAR.R).
...
pbmc <- readRDS("pbmc.chromvar.annotated.rds")
```
...
```{r, eval = FALSE}
file <- "pbmc.chromvar.annotated.rds"
