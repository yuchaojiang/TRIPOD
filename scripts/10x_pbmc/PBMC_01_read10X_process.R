...
nCount_ATAC=pbmc$nCount_ATAC
save(nCount_ATAC, file='pbmc.nCount_ATAC.rda')
nCount_RNA=pbmc$nCount_RNA
save(nCount_RNA, file='pbmc.nCount_RNA.rda')
...
# Platelet
VlnPlot(pbmc, features = c("rna_PPBP"), slot = "counts", log = TRUE, group.by = 'celltype')
# DC
...
# Make a plot within a region
DefaultAssay(pbmc) <- "ATAC"
celltype.names <- levels(pbmc)
CoveragePlot(pbmc, region = 'CD8A', features = 'CD8A', assay = 'ATAC', expression.assay = 'SCT', peaks = FALSE)
...
# We can see that both the expression of the CEBPB, 
# and the accessibility of the MA0466.2.4 motif 
# (which encodes the binding site for CEBPB), are both enriched in monocytes.
# returns MA0466.2
...
save(pbmc, file='pbmc_chromvar.rda')
