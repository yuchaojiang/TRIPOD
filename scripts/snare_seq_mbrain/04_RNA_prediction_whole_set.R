...
celltype_presto <- presto:::wilcoxauc.Seurat(X = snare, group_by = 'seurat.celltype', assay = 'data', seurat_assay = 'SCT')
celltype_genes=celltype_presto[order(celltype_presto[,'pval'])[1:1000],'feature'] # cell-type markers
highvar_genes=snare@assays$SCT@var.features[1:1000]
genes=unique(c(celltype_genes, highvar_genes))

# Visualize reduced dimensions and some gene markers
p1=DimPlot(snare, reduction = "umap.atac", label = TRUE, label.size = 2.5, repel = TRUE)+NoLegend()+ggtitle('ATAC reduced dim.')
p2=DimPlot(snare, reduction = "umap.rna",  label = TRUE, label.size = 2.5, repel = TRUE)+NoLegend()+ggtitle('RNA reduced dim.')
p3=DimPlot(snare, reduction = "wnn.umap", label = TRUE, label.size = 2.5, repel = TRUE)+NoLegend()+ggtitle('WNN reduced dim.')
p4=DimPlot(snare, reduction = "wnn.umap", group.by='seurat_clusters', label = TRUE, label.size = 2.5, repel = TRUE)+NoLegend()+ggtitle('WNN partition')
ggsave(filename ='snare_whole_set_reduced_dim.pdf',
       plot = p1+p2+p3+p4+ plot_layout(ncol=2),
       width = 8,
       height = 8)

p3=DimPlot(snare, reduction = "wnn.umap", label = TRUE, label.size = 2.5, repel = TRUE)+NoLegend()+ggtitle('WNN reduced dim.')
ggsave(filename='snare_whole_set_celltype.pdf',
       plot=p3,
       width=5, height=5)

p3=DimPlot(snare, reduction = "wnn.umap", label = TRUE, label.size = 2.5, repel = TRUE)+ggtitle('WNN reduced dim.')
ggsave(filename='snare_whole_set_celltype_legend.pdf',
       plot=p3,
       width=12, height=12)
...
for(dist in c(2000, 5000, 20000, 50000, 200000)){
  ...
  for(gene.name in genes){
    ...
    for(m in 1:(length(modelNames)-1)){
      ...
      if(ncol(xymats$Xit)==0 | ncol(xymats$X)==0 | sum(xymats$Y!=0) <=5) break # No peak overlap with the region
    ...
    if(i >=400) break # Here I only looked at the first 600 genes within each dist.
  }
  # remove NAs
...
for(dist in c(2000, 5000, 20000, 50000, 200000)){
  ...
  pdf(file=paste0('prediction/RNA_prediction_',gene.name,'_',dist,'.pdf'), width=12, height=3)
  par(mfrow=c(1,4))
  ...
  legend('bottomright',paste('r =',round(cor(xymats$Y, wnn.gene.maestro[,gene.name], method = 'pearson'),3)))
...
ggsave(filename='snare_pred_cor.pdf',
       plot=p,
       width=5, height=3)
